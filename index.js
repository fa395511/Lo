...
client.on('interactionCreate', async (modal) => {
    if (modal.customId === 'thstar1') {
        ...
        ctx.font = "24px Arial";
        ctx.fillText("From:  " + modal.user.username, 150, 50, 100);
        ...
        paragraphs.forEach((paragraph, index) => {
            const yPos = 85 + index * 35;
            ctx.fillText(paragraph, 200, yPos, 220);
        });
        const userAvatar = await loadImage(modal.user.avatarURL({ extension: 'png' }));
        const avatarCanvas = createCanvas(80,80);
        const avatarCtx = avatarCanvas.getContext("2d");
        const radius = 10;
        avatarCtx.beginPath();
        avatarCtx.moveTo(radius, 0);
        avatarCtx.lineTo(avatarCanvas.width - radius, 0);
        avatarCtx.quadraticCurveTo(avatarCanvas.width, 0, avatarCanvas.width, radius);
        avatarCtx.lineTo(avatarCanvas.width, avatarCanvas.height - radius);
        avatarCtx.quadraticCurveTo(avatarCanvas.width, avatarCanvas.height, avatarCanvas.width - radius, avatarCanvas.height);
        avatarCtx.lineTo(radius, avatarCanvas.height);
        avatarCtx.quadraticCurveTo(0, avatarCanvas.height, 0, avatarCanvas.height - radius);
        avatarCtx.lineTo(0, radius);
        avatarCtx.quadraticCurveTo(0, 0, radius, 0);
        avatarCtx.closePath();
        avatarCtx.clip();
        
        avatarCtx.drawImage(userAvatar, 0, 0, avatarCanvas.width, avatarCanvas.height);
        
        ctx.drawImage(avatarCanvas, 5, 15, 140, 140);
